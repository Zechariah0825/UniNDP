...
def main():
    ...
    # set up log file
    output_dir_name = os.path.join('pruning_and_breakdown', args.outdir) # "debug_test" # NOTE: you can change the output dir name
    ...
    gen_code, baseline_inst_count, predict_result = \
    ...
    """
    if args.quicksearch: # TODO: for pruning speedup experiment and breakdown experiment, so provide the detail result
        ...
        # writer.writerow([args.name, args.workload, best_result] + best_design + 
        ...
        writer.writerow(['optimal', best_result] + inst_count)
        csvfile.flush()
    
    else: # TODO: for pruning analysis
        print(f"search using brute force, size = {len(design_space)}, cmdthre = {args.cmdthre}")
        writer.writerow(['performance', 'performance upper bound', 'parallelism'])
        min_codelen = {}
        best_design = []
        best_result = inf
        for index in tqdm.tqdm(range(len(design_space))):
        ...
        # writer.writerow([args.name, args.workload, best_result] + best_design + 
        #                     [baseline_sim_result, baseline_inst_num, baseline_pu_dram_num, baseline_host_dram_num, baseline_row_change_num])
                performance = 1/sim_result
                upper_bound = 1/(SimConfig.col_change_apox*inst_count[1]+SimConfig.read_row_change_apox*inst_count[2])
                parralelism = mul(mul(partition[i]) for i in range(4))
                writer.writerow([performance, upper_bound, parralelism])
                csvfile.flush()
